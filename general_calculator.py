...
def run(
    ...
    wastages = [(sum(possible_lengths[opt])-width) for opt in range(len(possible_lengths))]
    ...
    if selected_option == 'Add Custom':
        ...
        num_pieces_custom = st.number_input('Enter number of product lengths in this combination:',
                                            key=num_pieces_key,
                                            step=1)
        combination = []
        for i in range(int(num_pieces_custom)):
            custom_piece_key = f'custom_piece_key_{window}_{i}'
            piece_length = st.number_input('Enter length for piece {} in mm'.format(i+1),
    ...
    total_product_length = (width*divisions)/1000 # in metres
    ...
    carrier_distances_per_piece = [
        [round(num, 2) for num in sublist] for sublist in carrier_distances_per_piece
    ]
    ...
    carrier_table[
        'Total length of {x} required'.format(
    ...
    return total_product_length, total_carrier_length, total_carrier_divisions
